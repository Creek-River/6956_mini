...
ARockEmitter::ARockEmitter()
{
	...
	static ConstructorHelpers::FClassFinder<ARock> smActor(TEXT("/Game/DungeonExplorer/GraphicAssets/BP_rock.BP_rock_C"));
...
void ARockEmitter::SpawnRock() {
	...
	*/
	...
	FVector pos = FVector(FMath::RandRange(-800, 800), FMath::RandRange(-300, 300), 0.0f);
	FRotator rot = FRotator(FMath::RandRange(-180, 180), FMath::RandRange(-180, 180), FMath::RandRange(-180, 180));

	// 算出値をもとに岩をスポーンさせる
	GetWorld()->SpawnActor<ARock>(rockActor, GetActorLocation() + pos, GetActorRotation() + rot, params);
